...
func FindPlayerByTag(playerTag string) (Player, bool) {
	//Writing player name in lowercase, replacing space with %20, trimming whitespace
	...
	if len(playerInfo.Players) == 0 { //separate function
		fmt.Println("Could not find any players with that name, please try again")
		return Player{}, false
	} else if len(playerInfo.Players) > 1 { //separate function
